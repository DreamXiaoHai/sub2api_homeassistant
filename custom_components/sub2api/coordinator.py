"""DataUpdateCoordinators for sub2API quota and token data."""
...
from .models import DashboardStats, Subscription
...
class Sub2APIUsageDataUpdateCoordinator(DataUpdateCoordinator[DashboardStats]):
    """Coordinate account-level dashboard token statistics."""
    ...
    async def _async_update_data(self) -> DashboardStats:
        try:
            return await self.client.async_get_dashboard_stats()
        except Sub2APIAuthError as err:
            raise ConfigEntryAuthFailed("sub2API authentication expired") from err
        except (Sub2APIError, ValueError) as err:
            raise UpdateFailed(str(err)) from err
